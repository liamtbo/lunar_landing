...
class ActionValueNetwork:
    ...
    def get_action_values(self, s):
        ...
        """ex: 1x6 * 6x12 = 1x12, first column of 6x12 is all the weights connected
                to the first node, so inputs * first column of weights = first node
        """
        psi = np.dot(s, W0) + b0
        x = np.maximum(psi, 0)
    ...
    def get_TD_update(self, s, delta_mat):
        """
        implements a temporal-difference (TD) update for a two-layer neural network.
        TD error is the loss function.
        
        Args:
            s (Numpy array): The state. can be more then 1 if batch size > 1!
        ...
        psi = np.dot(s, W0) + b0 # raw output of hidden
        x = np.maximum(psi, 0) # ReLU activation, hidden layer activations, (batch size * hidden layer neurons)
        dx = (psi > 0).astype(float) # gradient of RelU activation ... boolean array of shape (batch size, hidden layer neurons)
        ...
        # TD error for output layer
        ## r + gamma * q(s_t+1, a_t+1, w) - q(s,a, w)
        v = delta_mat # shape (batch_size, num_actions)
        # calculate updates for output layer
        """        
        np.dot(x.T,v)
            each activation * the q(s,a,w) TD error
            (x.T dot v) = (hidden neurons, batch) dot (batch, num_actions) = (hidden neurons, num_actions)
            ex
            [[a], dot [0, d] = [[0,ad], # state_1 did action 2, result is gradient of neurone 1 & 2 weight to output action 2
             [b]]             [0,bd]]
        1. / s.shape[0]
            s.shape[0] = number of samples in batch
            performs averaging across the batch dimension, resulting in an update that 
            reflects the average contribution of each weight across all samples in the batch.
        np.sum(v, axis=0, keepdims=True)
            axis=0
                0th dimension is the batch dimension
                sums the elements within each column, effectively calculating the total 
                error for each action across all samples in the batch.
            keepdims=True
                sum reduced original array dimensions by 1
                keepdims=True maintains dims of orignal array
                ex: 4x2 -> 1x2 (both are 2D)
        """
        ...
        """
        np.dot(v,W1.T)
            (batch, actions) dot (actions, neurons) = (batch, neurons)
            for each weight in layer 2 that is connected to samples action, multiply it by the error
            [0,0,a,0] dot [[w_11, w_21, ...], = [a * w_13, a * w_23, ...]
                         [w_12, w_22, ...],
                         [w_13, w_23, ...],
                         [w_14, w_24, ...]]
            a * w_13 -> error of state action * weight between hidden neuron 1 and output neuron 3
        dx
            gradient of RelU activation, boolean,
            shape (batch size, hidden layer neurons)
            acts as a gate that only allows gradients to flow through for weights connected to active neurons.
        np.dot(v,W1.T) * dx
            calculates how much the TD error for each action (in v) can be attributed to the activations 
            of each neuron in the first layer (layer 0)
            (batch, neurons) * (batch, hidden neurons)
            v = [a * w_13, a * w_23, ...] unless weigths hidden layer neurons activation is < 0,
            then it's spot in v = 0
        """
        v = np.dot(v, W1.T) * dx
...
class Adam():
    ...
    def __init__(self, layer_sizes, 
        ...
        # beta_m_product and beta_v_product should be ... ~0.1? (Note that timesteps start at 1 and if we were to 
...
#     print(f"preferences: {preferences}")
    ...
    max_preference = np.max(preferences, axis=1) # if true we dont need next line
#     print(f"max_preferences: {max_preference}")
    # your code here
...
#     print(f"reshaped_max_preference: {reshaped_max_preference}")
    # Compute the numerator, i.e., the exponential of the preference - the max preference.
    exp_preferences = np.exp(preferences - reshaped_max_preference)
#     print(f"exp_preferences: {exp_preferences}")
    # Compute the denominator, i.e., the sum over the numerator along the actions axis.
    sum_of_exp_preferences = np.sum(exp_preferences, axis=1)
#     print(f"sum_of_exp_preferences: {sum_of_exp_preferences}")
    # your code here
...
#     print(f"reshaped_sum_of_exp_preferences: {reshaped_sum_of_exp_preferences}")
...
#     print(f"action_probs_squeezed: {action_probs}")
    return action_probs
...
def get_td_error(states, next_states, actions, rewards, discount, terminals, network, current_q, tau):
    ...
    ### END CODE HERE
    # your code here
    next_action_sum = np.sum(q_next_mat * probs_mat, axis=1)
    v_next_vec = next_action_sum * (1 - terminals)
    ...
    # each element in Batch Indices is populated by its corresponding index
    batch_indices = np.arange(q_mat.shape[0])

    # grabbing q(s,a) of just the actions taken in states
...
def optimize_network(experiences, discount, optimizer, network, current_q, tau):
    ...
    """
    pre-concet states looks like: batch arrays with shape(1,state params)
        [array([[1,2,...]], datatype=float32), array([[1,2,...]], datatype=float32), ...]
    post_concat states looks like: shape (batch, state params)
        array([1,2,3,..], [1,2,3,...], ...)
    """
